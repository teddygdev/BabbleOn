...
public:
    ...
    SocketStreamBuffer(TCPSocket *sock) {
        ...
        std::streambuf::setg(inBuffer, inBuffer + sizeof(inBuffer),
             inBuffer + sizeof(inBuffer));
        std::streambuf::setp(outBuffer, outBuffer + sizeof(outBuffer));
        extra = 0;
    }
...
protected:
    ...
    int_type underflow() {
        ...
        // character read.
        std::streambuf::setg(inBuffer, inBuffer, inBuffer + len / sizeof(CharT));
        return this->sgetc();
    }
...
throw(SocketException) {
    ...
    int n = 1;
    setsockopt(sockDesc, SOL_SOCKET, MSG_NOSIGNAL, &n, sizeof(n));
    //
